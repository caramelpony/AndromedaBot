...
class TSMMat4 {
    ...
    static lookAt(position, target, up = vec3_1.TSMVec3.up) {
        if (position.equals(target)) {
            return this.identity;
        }
        const z = vec3_1.TSMVec3.difference(position, target).normalize();
        const x = vec3_1.TSMVec3.cross(up, z).normalize();
        const y = vec3_1.TSMVec3.cross(z, x).normalize();
        return new TSMMat4([
            x.x,
            y.x,
            z.x,
            ...
            z.z,
            0,
            -vec3_1.TSMVec3.dot(x, position),
            -vec3_1.TSMVec3.dot(y, position),
            -vec3_1.TSMVec3.dot(z, position),
            1
        ]);
    }
    ...
    multiplyTSMVec3(vector) {
        const x = vector.x, y = vector.y, z = vector.z;
        return new vec3_1.TSMVec3([
    ...
    multiplyTSMVec4(vector, dest = null) {
        if (!dest) {
            dest = new vec4_1.TSMVec4();
        }
...
exports.TSMMat4 = TSMMat4;
TSMMat4.identity = new TSMMat4().setIdentity();
